\begin{tikzpicture}[scale=1.33, line width=0.6pt, every node/.style={ellipse, draw, fill=white}, >=latex]
    ...
    \node [fill=black!25] (obs) at (2, 2) {$\log\teff[i], \log L_i$} edge [<-] (teff) edge [<-] (rad);
    ...
    \fill (3.8, 2) circle (1.5pt) edge [->] (obs) node[above, fill=none,draw=none] {$\sigma_{\teff, i}, \sigma_{L,i}$};
